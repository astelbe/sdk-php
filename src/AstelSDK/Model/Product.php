...
class Product extends SDKModel {
	...
	/**
	 * @param $products
	 *	Order product on full price even if discounted price is available)
	 * @return mixed
	 */
	...
	/**
	 * @param $products
	 * Order product on discounted price if available
	 * @return mixed
	 */
	public function orderByDiscountedPrice($products) {
		// Order by displayed price
		foreach ($products as $k => $product) {

			// Regular price by default
			$products[$k]['price_for_order'] = $product['price'];

			// Ex 20€/month during 6 months instead of 25€
			// If discounted price period is superior to 0
			if($product['discounted_price_period'] > 0) {			
				$products[$k]['price_for_order'] = $product['discounted_price'];
			};
		...
		$ordered_products = Hash::sort($products, '{n}.price_for_order', 'asc');
		return $ordered_products;
	}
