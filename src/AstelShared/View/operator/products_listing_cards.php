  ...
  ?>
    <div class="productCard">
      ...
      ?>
        ...
        <img src="<?= $item['brand_logo'] ?>" alt="<?= $item['brand_name'] ?>" class="productCard-brandLogo" style="max-height:32px;">
...
<style>
  ...
  /* override translation key class */
  .product-plays .text-blue {
    ...
    ?>
      <div class="col-12 col-xl-3 col-lg-4 col-md-6 mb-5 px-1 mb-5 mt-4 product-card">
        <!-- <?php if ($result['result_index']) { ?>
          ...
          <?php } ?> -->
        <div class="px-2 pt-3 pb-2 rounded-lg d-flex h-100 flex-column justify-content-between align-item-end" style="box-shadow: 2px 0rem 1.2rem rgba(0,0,0,.35)!important">
          <?php if ($cashback) { ?>

            <div class="py-2 px-3 mb-3 text-center cursor-pointer rounded-xl plugin-hidden-optional-element cashback-amount modalClick " data-toggle="modal" data-target="#pluginModalCashback" style="color:#fff; background-color: #E5176B; margin:0 auto; border-radius: 0.5rem">
          ...
          <?php } ?>
            ...
            foreach ($result['products'] as $key => $item) {
              ...
              if (($cpt == 1 || ($cpt == 2 && $params['id'] == 'view_multi_brand')) && $params['options']['display_operator_in_product_name'] !== false) { ?>
                  <div class="mb-3" style="height: 36px">
                    <img src="<?= $item['brand_logo'] ?>" alt="<?= $item['brand_name'] ?>" style="max-height:32px;">
                  </div>
              <?php } ?>
              <?php if ($item['product_sheet_url'] != '') { ?>
                ...
                <?php } ?>
                <h3 class="px-1 pt-3 d-flex justify-content-between text-center" <?= ($cpt == 1 ? 'style="min-height: 80px; font-size: 1.1rem;"' : '') ?>>
                ...
                <?php } ?>
              <div class="pt-2 px-1 mb-1" style="background-color: <?= $item['brand_bg_color'] ?> ">
                <?php foreach ($item['plays'] as $k => $play) {
                  if ($play !== false) { ?>
                    <div class="d-flex align-items-baseline pb-1" style="line-height:25px;font-size:0.875rem;">
                      <div class="mr-1">
                        <span style="display:inline-block; width:30px"><?= $play['label'] ?></span>
                      </div>
                      <div class="product-plays">
                        <?= $play['details'] ?>
                      </div>
                    </div>
          ...
          <div class="results-price d-flex text-center flex-column justify-content-center mt-auto">
